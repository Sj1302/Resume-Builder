          ...
          <React.Fragment>
            {data ? (
              <motion.div onClick={()=>setIsMenu(!isMenu)} {...FadeInandOut}>
                ...
                <AnimatePresence>
                  {isMenu && (
                    <motion.div onMouseLeave={()=>setIsMenu(false)} 
                    {...SlideUpDownMenu}
              ...
              <Link to={"/auth"}>
                <motion.button {...FadeInandOut} className="px-4 py-2 rounded-md border border-gray-200 bg-gray-200 hover:shadow-md active:scale-95 duration-150" type="button">Login</motion.button>
              </Link>
            )}
          </React.Fragment>
        )}
